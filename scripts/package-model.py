...
def package_model(model_path: Path, core_report: Path, output: Path) -> dict:
    ...
    if any(name.lower() == "release-metadata.json" for name in source_files):
    ...
    for name in source_files:
        if any(parent.as_posix() in source_files for parent in Path(name).parents if str(parent) != "."):
    ...
    try:
        ...
        metadata = {
            ...
            "scope": "Runtime dependencies copied after structural checks and exact supplied official-Core report matching. No target-application acceptance is inferred.",
        }
        ...
        return {"passed": True, "output": str(output), "zip": str(archive),
                ...
                "runtimeFiles": len(files), "nativeCoreValidated": True,
                "vtubeStudioTested": False}
    finally:
        if stage.exists():
            shutil.rmtree(stage)
