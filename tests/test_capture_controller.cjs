...
async function fixture(){
 ...
 const descriptors=[{id:'ParamAngleX',min:-30,max:30,default:0},{id:'ParamEyeLOpen',min:0,max:1,default:1},{id:'ParamEyeROpen',min:0,max:1,default:1},{id:'ParamBrowLY',min:-1,max:1,default:0},{id:'ParamBrowRY',min:-1,max:1,default:0},{id:'ParamBodyAngleZ',min:-10,max:10,default:0},{id:'ParamBreath',min:0,max:1,default:0}];
 let params=Object.fromEntries(descriptors.map(p=>[p.id,p.default])),released=0;
 const viewer={getParameters:()=>descriptors,getState:()=>({parameters:{...params}}),setCaptureParameters:p=>Object.assign(params,p),clearCaptureParameters(){},clearParameters(){},setParameter:(id,v)=>params[id]=v,renderNow(){},setAutoPlay(){},setPointerFollow(){},setExpression(){}};
 const controller=new CaptureController({getViewer:()=>viewer,onSourceStop:()=>released++});controller.setOptions({smoothing:0});
 return {controller,viewer,params,get released(){return released;}};
}
